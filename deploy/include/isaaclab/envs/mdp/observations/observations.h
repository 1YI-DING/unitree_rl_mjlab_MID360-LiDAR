...
{
    return isaaclab::mdp::last_action(env, params);
}
...
REGISTER_OBSERVATION(velocity_commands)
{
    ...
    const std::string command_name = params["command_name"].as<std::string>("base_velocity");
    const auto cfg = env->cfg["commands"][command_name]["ranges"];
...
{
    return isaaclab::mdp::gait_phase(env, params);
}

REGISTER_OBSERVATION(height_scan)
{
    const int size = params["size"].as<int>(0);
    const float miss_value = params["miss_value"].as<float>(5.0f);

    auto & asset = env->robot;
    std::lock_guard<std::mutex> lock(asset->data.height_scan_mutex);

    if (asset->data.height_scan.size() == static_cast<size_t>(size)) {
        return asset->data.height_scan;
    }

    return std::vector<float>(size, miss_value);
}
